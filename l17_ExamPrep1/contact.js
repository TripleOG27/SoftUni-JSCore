class Contact{
    ...
    let articleDiv = $('<article>')
        let tittleDiv = $(`<div class="title">${this.firstName} ${this.lastName}</div>`)
        let infoButton = $('<button>&#8505;</button>')
            .click(()=>infoDiv.toggle())
        let infoDiv = $(`<div class="info" style="display: none">
 <span>&phone; ${this.phone}</span>
 <span>&#9993; ${this.email}</span>
 </div>`)
        articleDiv.append(tittleDiv.append(infoButton)).append(infoDiv)
        return articleDiv
    }
}
